...
import { GUJARAT_POLICY_DATA } from './gujaratConstants';
import { Hotel, ItineraryPackage, RoomType, Sightseeing } from '../../types';
import { FleetItem, ItineraryPricing } from '../itinerary/types';
import { FALLBACK_IMG, getMealPlanLabel, getSmartDate, getVehicleString } from '../itinerary/utils';

interface GujaratPdfTemplateProps {
  guestName: string;
  pax: number;
...
export const GujaratPdfTemplate: React.FC<GujaratPdfTemplateProps> = ({
  ...
  return (
    <div id="gujarat-pdf-template-container" className="fixed top-0 left-0 -z-50 bg-white text-slate-900 font-sans pointer-events-none">

      {/* --- COVER --- */}
      <div className={cn("pdf-section relative h-[1123px] flex flex-col justify-between overflow-hidden", PDF_WIDTH_CLASS)}>
        <div className="absolute inset-0 z-0">
          <img
            src="https://images.unsplash.com/photo-1609948543911-7280795f813d?auto=format&fit=crop&w=1200&q=80"
            className="w-full h-full object-cover"
            alt="Cover"
            crossOrigin="anonymous"
            loading="eager"
          />
          <div className="absolute inset-0 bg-gradient-to-t from-amber-950/90 via-amber-900/30 to-amber-800/10" />
        </div>
        ...
        <div className="relative z-10 p-12 pt-20 flex justify-between items-start">
          <div className="text-white">
            ...
            <h1 className="text-7xl font-serif font-bold leading-[0.9]">
              Gujarat<br /><span className="text-amber-400">Odyssey</span>
            </h1>
          </div>
          <div className="text-white text-right">
            <p className="text-xl font-serif italic text-white/80">A Curated Journey Through</p>
            <p className="text-3xl font-bold tracking-wide">The Land of Legends</p>
          </div>
        </div>
            ...
            <div className="grid grid-cols-2 gap-12 text-sm border-t border-white/20 pt-8">
              <div>
                ...
                <p className="font-bold text-xl flex items-center gap-2">
                  <Calendar size={20} className="text-amber-400" /> {getSmartDate(startDate, 0, true)} — {getSmartDate(startDate, activePackage.days - 1, true)}
                </p>
              </div>
              <div>
        ...
        <div className="flex gap-10 h-72">
          <div className="w-1/2 flex flex-col justify-between">
            <div>
              ...
              <span className="text-slate-400 font-medium italic text-sm">Your personalized Gujarat journey at a glance</span>
                ...
                {activePackage.route.map((city, idx) => (
                  <React.Fragment key={idx}>
                    <div className="flex flex-col items-center gap-2">
                      <div className="w-3 h-3 rounded-full bg-amber-600 ring-4 ring-white shadow-md"></div>
          ...
          <div key={index} className={cn("pdf-section p-8 pt-4 pb-4 bg-white border-b border-slate-50 break-inside-avoid", PDF_WIDTH_CLASS)}>
            <div className="flex gap-6 h-full">
              <div className="flex flex-col items-center w-12 shrink-0">
                <div className="w-10 h-10 rounded-full bg-amber-600 text-white flex items-center justify-center font-serif font-bold text-lg shadow-lg z-10">
                  {index + 1}
                </div>
                <div className="w-0.5 bg-dashed border-l-2 border-amber-200 h-full -mt-2 opacity-50"></div>
              </div>
              ...
              <div className="flex-1 pb-8">
                <div className="flex justify-between items-baseline mb-4">
                  ...
                  {index === 0 && <span className="bg-amber-100 text-amber-800 text-[10px] font-bold px-2 py-1 rounded uppercase">Arrival</span>}
        ...
        <div className="bg-white rounded-3xl shadow-xl p-8 border border-slate-200">
          <div className="flex justify-between items-end border-b border-dashed border-slate-200 pb-6 mb-6">
            ...
            <div className="text-right">
              ...
              <p className="text-xl font-bold text-amber-600">{formatCurrency(pricing.perPerson)}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-12">
            <div>
              ...
              <ul className="space-y-2">
                {GUJARAT_POLICY_DATA.inclusions.map((inc, i) => (
            ...
            <div>
              ...
              <ul className="space-y-2 opacity-60">
                {GUJARAT_POLICY_DATA.exclusions.map((exc, i) => (
                  <li key={i} className="text-sm text-slate-600 flex items-start gap-2">

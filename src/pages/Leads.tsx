                    ...
                    <table className={cn("w-full text-left border-collapse", getTextColor())}>
                        ...
                        <tbody>
                            {filteredLeads.map(lead => (
                                <tr key={lead.id} className={cn("transition-colors", theme === 'light' ? 'border-b border-slate-100 hover:bg-slate-50' : 'border-b border-white/5 hover:bg-white/5')}>
                                    <td className="p-5 font-semibold">
                                        <div className="flex items-center gap-2">
                                            {lead.name}
                                            <DialButton phoneNumber={lead.contact.phone} className="w-11 h-11" />
                                        </div>
                                    </td>
                                    <td className="p-5">
                                    ...
                                    <td className="p-5 text-sm">{lead.tripDetails.destination}</td>
                                    <td className="p-5 text-sm font-mono tracking-wide">{formatCurrency(lead.tripDetails.budget)}</td>
                                    <td className="p-5">
                                        <Link to={`/leads/${lead.id}`} className="text-blue-500 hover:text-blue-400 font-medium text-sm">View</Link>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Create New Lead">
        <form onSubmit={handleAddLead} className="space-y-5">
            ...
            <div className={cn("p-4 rounded-xl border border-dashed space-y-3", getBorderClass(), theme === 'light' ? 'bg-slate-50' : 'bg-white/5')}>
                ...
                {/* Replaced Destination/Budget/StartDate/Pax inputs with specific layout */}
